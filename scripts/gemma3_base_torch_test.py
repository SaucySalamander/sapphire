...
def run_autoregressive(input_ids, attention_mask, max_new_tokens=20, do_sample=False, temperature=1.0, tag="greedy"):
    ...
    # Detect common stop tokens (EOS, pad, or token 1 which is often EOS)
    stop_tokens = {1}  # EOS is usually token 1
    ...
    for step in range(max_new_tokens):
        ...
        # stop if EOS or recognized stop token
        if tok_id in stop_tokens:
            print(f"GEN: Stopping early at step {step} (token {tok_id} is stop token)")
            break
